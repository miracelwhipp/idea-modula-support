// This is a generated file. Not intended for manual editing.
package org.modula.parsing.definition.psi;

import java.util.List;
import org.jetbrains.annotations.*;
import com.intellij.psi.PsiElement;

public interface DefinitionDefinitions extends PsiElement {

  @NotNull
  List<DefinitionConstantDeclaration> getConstantDeclarationList();

  @Nullable
  DefinitionProcedureHeading getProcedureHeading();

  @NotNull
  List<DefinitionVariableDeclaration> getVariableDeclarationList();

  @NotNull
  List<DefinitionTypeDefinition> getTypeDefinitionList();

}
